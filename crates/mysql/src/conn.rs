...
use crate::arrow_sql_gen::{self, map_column_to_data_type, rows_to_arrow};
...
pub enum Error {
    ...
    ConversionError { source: arrow_sql_gen::Error },
...
impl<'a> AsyncDbConnection<Conn, &'a (dyn ToValue + Sync)> for MySQLConnection {
    ...
    > {
            ...
            Err(e) => match e {
                mysql_async::Error::Server(server_error) => {
                    if server_error.code == 1146 {
                        return Err(datafusion_table_providers_common::sql::db_connection_pool::dbconnection::Error::UndefinedTable {
        ...
        Ok(columns_meta_to_schema(columns_meta).context(datafusion_table_providers_common::sql::db_connection_pool::dbconnection::UnableToGetSchemaSnafu)?)
    }
