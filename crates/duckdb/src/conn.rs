    ...
    for DuckDbConnection
{
    ...
    > {
        ...
        while let Some(row) = rows.next().boxed().context(datafusion_table_providers_common::sql::db_connection_pool::dbconnection::UnableToGetTablesSnafu)? {
            tables.push(row.get(0).boxed().context(datafusion_table_providers_common::sql::db_connection_pool::dbconnection::UnableToGetTablesSnafu)?);
        }
    ...
    > {
        ...
        while let Some(row) = rows
            ...
            .context(datafusion_table_providers_common::sql::db_connection_pool::dbconnection::UnableToGetSchemasSnafu)?
        {
            schemas.push(row.get(0).boxed().context(datafusion_table_providers_common::sql::db_connection_pool::dbconnection::UnableToGetSchemasSnafu)?);
        }
